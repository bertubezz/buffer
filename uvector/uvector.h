...
public:
  ...
  /** @brief Construct a vector with given amount of elements, without initializing these.
	 ...
	 */
  explicit uvector(size_t n) : _begin(allocate(n)),
                               _end(_begin + n),
                               _endOfStorage(_end)
  {
  }
  ...
  /** @brief Construct a vector with given amount of elements and set these to a specific value.
	 ...
	 * @param allocator Allocator used for allocating and deallocating memory.
	 */
  uvector(size_t n, const value_type &val, const allocator_type &allocator = Alloc()) : Alloc(allocator),
                                                                                        _begin(allocate(n)),
                                                                                        _end(_begin + n),
                                                                                        _endOfStorage(_end)
  {
  ...
  /** @brief Copy construct a uvector with custom allocator.
	 ...
	 * @param allocator Allocator used for allocating and deallocating memory.
	 */
  uvector(const uvector<Tp, Alloc> &other, const allocator_type &allocator) : Alloc(allocator),
                                                                              _begin(allocate(other.size())),
                                                                              _end(_begin + other.size()),
                                                                              _endOfStorage(_end)
  {
  ...
  /** @brief Move construct a uvector.
	 ...
	 */
  uvector(uvector<Tp, Alloc> &&other) noexcept : Alloc(std::move(other)),
                                                 _begin(other._begin),
                                                 _end(other._end),
  ...
  /** @brief Move construct a uvector with custom allocator.
	 ...
	 * @param allocator Allocator used for allocating and deallocating memory.
	 */
  uvector(uvector<Tp, Alloc> &&other, const allocator_type &allocator) noexcept : Alloc(allocator),
                                                                                  _begin(other._begin),
                                                                                  _end(other._end),
                                                                                  _endOfStorage(other._endOfStorage)
  {
  ...
  /** @brief Construct a uvector from a initializer list.
	 ...
	 * @param allocator Allocator used for allocating and deallocating memory.
	 */
  uvector(std::initializer_list<Tp> initlist, const allocator_type &allocator = Alloc()) : Alloc(allocator),
